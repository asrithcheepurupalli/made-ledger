...
export function SpendTab() {
  ...
  const rows = useLiveQuery(
    () => db.transactions.where("date").equals(today).and((t) => t.type === "expense").reverse().sortBy("createdAt"),
    [today],
    [],
  );
  ...
  return (
    <div className="flex flex-col gap-6 px-4 py-5">
      <div>
        ...
        <p className="label mt-1 text-grey-dim">What the shop paid out today</p>
      </div>
      ...
      <div>
        ...
        <div className="mt-2 divide-y divide-ink-line border-t border-ink-line">
          {(rows ?? []).length === 0 && (
            <p className="py-4 text-sm text-grey-dim">No expenses logged yet today.</p>
          )}
          {(rows ?? []).map((t) => (
